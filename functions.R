#FROM https://github.com/cgls/Copernicus-Global-Land-Service-Data-Download-with-R/blob/master/land.Copernicus%20Data%20Download.R
############################################################################################################################
#
...
download.CGLS.data <- function(
  ...
  #setting timeout
  user.timeout <- getOption('timeout')
  options(timeout = as.integer(timeout))
  on.exit(options(timeout=user.timeout))
  
  #preparing 300m resolutions
  if(resolution == "300m"){
    resolution1 <- "333m"
  ...
  #generating the collection name
  collection <- paste(
    product, 
    version, 
    resolution1, 
  ...
  product.link <- paste0(
    "@land.copernicus.vgt.vito.be/manifest/", 
    collection, 
    "/manifest_cgls_", 
    collection, 
    "_latest.txt"
  ...
  #urls of all files in the manifest
  file.url <- getURL(
    url, 
    ftp.use.epsv = FALSE, 
    dirlistonly = TRUE, 
  ...
  #iterating through timeframes
  for (i in 1:length(timeframe)){
    
    #finding file wiht the given date
    file.link <- grep(
      gsub(
        "-", 
        "",
    ...
    #if the file is available, download
    if (length(file.link) > 0 ){
      
      #adding 3 waiting seconds to avoid server panick
      if (i>1){Sys.sleep(3)}
      ...
      # print(paste0(collection, "_", sub(".*/", "", file.link), " is saved in ", getwd()))
